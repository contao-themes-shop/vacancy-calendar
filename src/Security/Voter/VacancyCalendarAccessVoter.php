...
class VacancyCalendarAccessVoter extends AbstractDataContainerVoter
{
    ...
    protected function hasAccess(TokenInterface $token, CreateAction|DeleteAction|ReadAction|UpdateAction $action): bool
    {
        ...
        return match (true) {
            $action instanceof CreateAction => $this->accessDecisionManager->decide($token,
                [VacancyCalendarPermissions::USER_CAN_CREATE_VACANCY_CALENDAR]),
            $action instanceof ReadAction,
                $action instanceof UpdateAction => $this->accessDecisionManager->decide($token,
                [VacancyCalendarPermissions::USER_CAN_EDIT_VACANCY_CALENDAR], $action->getCurrentId()),
            $action instanceof DeleteAction => $this->accessDecisionManager->decide($token,
                    [VacancyCalendarPermissions::USER_CAN_EDIT_VACANCY_CALENDAR], $action->getCurrentId())
